...
export function NotificationsSettings() {
  const { enabled, reminders, permission, pushEnabled, pushSupported, setEnabled, addReminder, updateReminder, removeReminder, requestPermission, testNotification, enablePush, disablePush } = useNotifications();
  ...
  const togglePush = async () => {
    ...
    try {
      if (pushEnabled) await disablePush();
        ...
        if (!ok) setPushError("No se pudo activar el push (revisa las claves VAPID).");
      }
    } catch (e) {
    ...
    <section className="rounded-xl border border-neutral-200 bg-white p-6 shadow-sm">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-neutral-500">Recordatorios al sistema (PC/móvil) mientras la app esté abierta.</p>
        </div>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3 rounded-xl bg-neutral-50 p-3 text-sm">
        {permission === "unsupported" ? (
          ...
          <>
            ...
            <button onClick={testNotification} className="rounded-lg border border-neutral-300 px-3 py-1.5 text-sm hover:bg-neutral-100">Probar notificación</button>
          </>
        ) : permission === "denied" ? (
          <span className="text-amber-700">Permiso bloqueado. Actívalo en los ajustes del navegador para este sitio.</span>
        ) : (
      ...
      {enabled && permission !== "granted" && (
        <p className="mt-2 text-xs text-amber-600">Concede el permiso para que los recordatorios se muestren.</p>
      )}
      ...
      {permission === "granted" && pushSupported && (
        <div className="mt-3 flex flex-wrap items-center gap-3 rounded-xl border border-neutral-200 p-3 text-sm">
          <div className="min-w-0 flex-1">
            <p className="font-medium">Avisos con la app cerrada (push)</p>
            <p className="text-xs text-neutral-500">
              {pushEnabled ? "Activado en este dispositivo. Recibirás avisos aunque cierres la web." : "Recibe recordatorios aunque la web esté cerrada."}
            </p>
          ...
          <button
            onClick={togglePush}
            disabled={pushBusy}
            className={`rounded-xl px-4 py-2 text-sm font-medium shadow-sm transition disabled:opacity-50 ${
      ...
      <div className="mt-5 space-y-3">
        ...
        {reminders.length === 0 ? (
          <p className="rounded-xl border border-dashed border-neutral-200 bg-neutral-50 p-4 text-center text-sm text-neutral-400">Sin recordatorios. Añade uno (p. ej. «Fichar entrada» o «Pedido proveedores»).</p>
        ) : (
            ...
            {reminders.map((r) => (
              <li key={r.id} className="rounded-xl border border-neutral-200 p-3">
                <div className="flex items-center gap-2">
                  ...
                  <input className={`${input} min-w-0 flex-1`} value={r.label} onChange={(e) => updateReminder(r.id, { label: e.target.value })} placeholder="Mensaje del recordatorio" />
      ...
      <p className="mt-4 text-xs text-neutral-400">
        Con «Activar push» recibirás los avisos aunque la web esté cerrada (requiere que el servidor ejecute la tarea programada). Sin push, los avisos se muestran mientras la app está abierta. En iPhone, instala la app en la pantalla de inicio para recibir notificaciones.
      </p>
    </section>
  );
}
